...
all_wilcox_against_null <- lapply(all_genes, function(current_gene){ 
  ...
  unobserved_allele_changes = all_nucleotide_changes[current_mutations, on = "mutated_from_allele==mutated_from_allele", allow.cartesian = TRUE, nomatch=0] %>%
  ...
  unobserved_allele_changes[, dummy_mutation_id := paste(chromosome, chromosome_start, chromosome_end, mutated_from_allele, mutated_to_allele, sep = "_")]
  
  #check that unobserved allele changes are not observed in other patients
  any_patient_unobserved_allele_changes = unobserved_allele_changes[!dummy_mutation_id %in% current_mutations[, dummy_mutation_id]]
  ...
  ism_predictions <- read_feather(glue("{ism_dir}/{current_gene}_ism_predictions.feather")) %>% as.data.table
  ism_predictions[ ,`:=`(chr = NULL, start = NULL, end = NULL, from = NULL, to = NULL)]
  ###
  #
  any_patient_unobserved_allele_changes[ism_predictions, on = "dummy_mutation_id==ID", delta_sure := diff_pred_avg]
  current_mutations[ism_predictions, on = "dummy_mutation_id==ID", delta_sure := diff_pred_avg]
  
  plot_data = rbind(current_mutations[,.(dummy_mutation_id, delta_sure)], any_patient_unobserved_allele_changes[,.(dummy_mutation_id, delta_sure)])
  plot_data[, label := ifelse(dummy_mutation_id %in% current_mutations[,dummy_mutation_id], "observed_mutation", "comparison_group")]
  if (grepl('_abs', current_mode)){
  ...
  write_path = glue("plotData_${model_pickle.simpleName}_{cancer_type}_{current_chromosome}_alternativeAlleles_out.tsv")
...
fwrite(all_wilcox_against_null, glue("${model_pickle.simpleName}_{cancer_type}_alternativeAlleles.tsv"), sep = "\\t")
